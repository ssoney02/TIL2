...
def create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # 게시글 작성을 요처하는 유저 객체!
            # 현재 유저가 누구인지 -> 이미 요청 객체에 어떤 사람인지 들어있기 때문에 조회하지 않아도 됨!
...
def update(request, pk):
    ...
    # 수정 요청 시도하는 사용자가 작성자가 아니라면 메인 페이지로 redirect
    else:
        return redirect('articles:index')
    context = {
        'article': article,
...
@login_required
@require_POST
# 댓글 생성 페이지를 따로 쓰지 않고 detail에 나오기 때문에 애초에 POST요청 밖에 안받음
