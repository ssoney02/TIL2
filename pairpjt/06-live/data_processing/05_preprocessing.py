...
def preprocessing():
    ...
    for book in books:
        ...
        if "description" in fields:
            ...
            # &lt; 와 &gt;를 제거하거나, 필요에 따라 HTML 태그 패턴 제거 (여기서는 단순 제거) - 진짜 눈에 보이는거 그냥 찾아서 지움
            # html에서 꺾쇠는 태그를 나타내는 형태라서 태그로 인식되지 않게 하기 위해 특수문자 처리
            processed_desc = re.sub(r'&lt;', '', original_desc)     # &lt; -> less than (<) 
            processed_desc = re.sub(r'&gt;', '', processed_desc)    # &gt; -> greater than (>)
